...
def all():
    ...
    d = '/dev/shm'
    data_dir1 = Path(d + '/gray_checked/')
    data_dir2 = Path(d + '/gray_wrong/')

    images1 = list(map(str, list(data_dir1.glob("*.jpg*"))))
    images2 = list(map(str, list(data_dir2.glob("*.jpg*"))))

    images = images1 + images2

    # images = images[1000:2000]
    ...
    x_train_w, x_valid_w, y_train_w, y_valid_w = split_data(np.array(images), np.array(labels))
    ...
    boost_training(x_train, y_train, x_valid, y_valid, epoches_batches=[(2,20), (1,6)])
    boost_training(x_train_w, y_train_w, x_valid_w, y_valid_w, epoches_batches=[(3,10), (3,1)])
